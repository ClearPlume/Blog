...
layui.define(['jquery'], function (exports) {
    ...
    Class.prototype.initStyle = function (_this) {
        ...
        style = ['<style type="text/css">',
            ...
            _this.selector, ' + .', extClassName, ' { position: relative; left: -129px; top: 35px; z-index: ', _this.options.zIndex, '; display: block; outline: none; }',
            _this.selector, ' + .', extClassName, ' * { color: ', (_this.options.color || '#666666'), '; }',
            _this.selector, ' + .', extClassName, ' a > i { font-size: 12px!important; }',
            _this.selector, ' + .', extClassName, ' a { padding: 0 3px; cursor: unset; }',
            _this.selector,
            ...
            _this.selector, ' + .', extClassName, '-l.', extClassName, '-inner > .layui-badge { border-radius: 15px 15px 0 0; margin-left: 1px; }',
            _this.selector, ' + .', extClassName, '-inner > .layui-badge  { top: -53px; border-bottom: 0 none; opacity: 0.8; }',
    ...
    Class.prototype.getToolsNode = function (_this, $target) {
        ...
        // 总是显示收起展开按钮
        tools.hide = ['<a href="javascript:;"', _this.getTips(_this, '**NO_TIP**'), 'class="', tools.hideClass, '"><i class="layui-icon layui-icon-more"></i></a>'].join('');
        // 至少显示一个工具模块
    ...
    Class.prototype.initTips = function (_this) {
        $('[' + tools.laytips + ']').each(function (i, n) {
            var $target = $(n);
            if ($target.attr(tools.laytips) !== '**NO_TIP**'){
